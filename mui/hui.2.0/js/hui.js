...
var hui = (function(selector, undefined){
	...
	huibase.ajax = function(sets){
		...
		if(!sets.url){return ;}
		var async = 'async' in sets ? settings.async : true;
	...
	/*
	普通消息提示框
	参数 : 1、消息内容 2、时间长短设置 默认 short，可选值 [short, long]
	*/
	...
	/*
	带有指定图标的消息框
	参数 : 1、消息内容 2、图标类型 [可选，默认right]（图标名称，去除 hui-icons后的名称）
	*/
	...
	*/
	...
	// 警告提示框 参数 : 1、消息内容 2、按钮名称[可选] 3、点击按钮后执行的回调函数[可选]
	...
	//确认提示框 参数 : 1、消息内容 2、按钮名称[数组类型，可选，默认['取消','确定']] 3、点击确定按钮后执行的回调函数
	 ...
	 * 参数 : 
	 * 1、消息内容 
	 * 2、按钮名称[数组类型，可选，默认['取消','确定']] 
	 * 3、点击确定按钮后执行的回调函数并传递输入内容到函数内
	 * 4、输入框内默认提示内容
	 */
